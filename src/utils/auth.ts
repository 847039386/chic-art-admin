...
export function setToken(data: DataInfo<Date>) {
  ...
  // 这里因为我后端的过期时间是获取秒。所以这里改为毫秒
  expires = new Date(Number(data.expires) * 1000).getTime(); // 如果后端直接设置时间戳，将此处代码改为expires = data.expires，然后把上面的DataInfo<Date>改成DataInfo<number>即可
