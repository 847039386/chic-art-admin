...
let addPermissionLoading = ref(false)
...
watch(
  () => [
    ...
    addPermissionLoading.value
  ],
  (newVal, oldVal) => {
    loading.value =
      !newVal[0] && !newVal[1] && !newVal[2] && !newVal[3] ? false : true;
  }
);
...
export const editPermission = async (action :string ,row?: any) => {
  ...
  onAddPermissionFormClick(action,row, function ( err ,results) {
